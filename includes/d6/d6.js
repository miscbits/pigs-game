...
D6Animator.getImageBank = function(key, baseUrl) {
	...
	if (!imageBank) {
		...
		imageBank.blank.src = baseUrl + "die-1.gif";
		var i;
		for (i=1; i<7; ++i) {
			whichDie = "die" + i;
...
D6AnimBuilder.prototype.genDiceHtml = function(layout, callback, callbackData) {
	...
	for (var i=0; i<layout.length; ++i) {
		...
		for (var j=0; j<imgsThisRow; ++j) {
			...
			if (dieCount > numTotalImgs) break;
			if (this.useImages) {
				genHtml += "<img id='" + this.id + dieCount + "' class='die' src='" + this.baseUrl + "die-1.gif' />";
			} else {
...
D6.dice = function(numDice, callback, callbackData, useImages, buttonLabel) {
	...
	if (!buttonLabel) buttonLabel = "Yes";
	...
	if (buttonLabel != "none") {
		genHtml += "<div class='diceform'><input type='button' id='dicebutton' value='" + buttonLabel + "' onclick='D6AnimBuilder.get(\"dice\").reset(); D6AnimBuilder.get(\"dice\").start()' /></div>";
		genHtml += "<div class='diceform'><input type='button' name='no'  value='no' id='no'></div>";
	}
	genHtml += "</div>";
	D6.genHtml = genHtml;
